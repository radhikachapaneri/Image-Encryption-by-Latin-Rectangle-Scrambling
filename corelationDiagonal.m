function corrvalueD =corelationDiagonal(image,numberOFPairs)
[row, col] = size(image);
totalPixels = row * col;
...
r = randi(totalPixels,1);
 while(mod(r,col) == 0 || (r >= (totalPixels - col) && (r<totalPixels)))
%             r = randint(1,1,[1,totalPixels]);
            r = randi(totalPixels,1);
 end   
 
        x(k) =  oneDimage(r);
        y(k) =  oneDimage(r+col+1);
        sumX = sumX + x(k);
        sumY = sumY + y(k);
...
cov = sumCov / numberOFPairs;
 ...
 corrvalueD = cov/(sqrt(DX)* sqrt(DY));
