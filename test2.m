...
%% Encryption
I=double(I);
S = RGBColorMixingRow2DLogistic(I,xy(:,1));
S = RGBColorMixingColumns2DLogistic(S,xy(:,2));
...
J = I;
J(1,1,:)=128;
S2 = RGBColorMixingRow2DLogistic(J,xy(:,1));
S2 = RGBColorMixingColumns2DLogistic(S2,xy(:,2));
...
% npcrG = NPCR(G2,G2two)
% npcrB = NPCR(B2,B2two)
R2 = double(R2); R2two = double(R2two);
G2 = double(G2); G2two = double(G2two);
B2 = double(B2); B2two = double(B2two);

resultsR = NPCR_and_UACI( R2, R2two, 0, 255 )
resultsG = NPCR_and_UACI( G2, G2two, 0, 255 )
resultsB = NPCR_and_UACI( B2, B2two, 0, 255 )
